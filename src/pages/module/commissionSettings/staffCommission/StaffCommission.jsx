...
export default function StaffCommission() {
  ...
  const fetchStaffCommission = async () => {
    setLoading(true);
    setTimeout(() => {
      setStaffCommission(dummyApiData);
      setLoading(false);
    }, 500);
  };

  const updateStaffCommission = async (values) => {
    setLoading(true);
    return new Promise((resolve) => {
      setTimeout(() => {
        console.log("UPDATED DATA:", values);
        setStaffCommission(values); // update UI
        setLoading(false);
        resolve(true);
      }, 800);
    });
  ...
  const handleUpdate = async (values) => {
    const isSuccess = await updateStaffCommission(values);
    if (isSuccess) {
      handleBack();
    }
  };
  ...
  return (
    <div className="min-h-screen">
      {currentPage === "commission-settings" && (
        <CommissionSetting
          onEdit={handleEdit}
          commissionData={formatCommissionData(staffCommission)}
          canUpdate={canUpdate}
        />
      )}
      {currentPage === "edit" && (
        <EditCommissionSetting
          initialData={formatCommissionData(staffCommission)}
          onBack={handleBack}
...
function CommissionSetting({ onEdit, commissionData, canUpdate }) {
  return (
    <div>
      ...
      />
      <PagePath2 title="Staff Commission" />

      <div className="space-y-6 bg-white pt-6 pb-8 rounded-xl">
        {/* Direct sale Section */}
        <Section title="Set Staff Commission for Direct Sale">
          <FieldView
            label="Quantity"
        ...
        <Section title="Set Staff Commission for Indirect Sale">
          <FieldView
            label="Quantity"
        ...
        {/* Lucky Draw Ticket */}
        <Section title="Set Lucky Draw Ticket on Purchase for Staff">
          <FieldView
            label="Quantity"
        ...
        <Section title="Set Withdraw Amount % Of Total Wallet Balance For Staff">
          <FieldView
...
function EditCommissionSetting({ initialData, onBack, onUpdate, loading }) {
  ...
  return (
    <div>
      <BreadCrumb
        linkText={[{ text: "Commission Setting" }, { text: "Edit Staff Commission" }]}
      />
      <PagePath2 title="Edit Staff Commission" />
